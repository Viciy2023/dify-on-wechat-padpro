...
export default function registerClawMateCompanion(api: OpenClawPluginApiLike): void {
  ...
  api.registerTool({
    ...
    description: "基于图片生成视频。需要提供参考图片URL和视频动作描述。视频时长6-30秒。",
    parameters: {
      type: "object",
      additionalProperties: false,
      ...
      properties: {
        baseImageUrl: {
          type: "string",
          description: "参考图片URL（必填），通常是之前生成的自拍图片URL"
        },
        prompt: {
          type: "string",
          description: "视频动作描述（必填），例如：'The woman in the image smiles and waves her hand'"
        },
        duration: {
          type: "number",
          ...
          return {
            content: [{
              ...
              text: JSON.stringify({
                ...
                provider: result.provider,
                message: "视频生成成功",
                mediaLine: `MEDIA: ${result.videoUrl}`
              })
            }]
          };
        } else {
