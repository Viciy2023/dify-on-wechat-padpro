    ...
    def _decorate_reply(self, context: Context, reply: Reply) -> Reply:
        if reply and reply.type:
            ...
            if not e_context.is_pass() and reply and reply.type:
                ...
                elif reply.type == ReplyType.IMAGE_URL or reply.type == ReplyType.VOICE or reply.type == ReplyType.IMAGE or reply.type == ReplyType.FILE or reply.type == ReplyType.VIDEO or reply.type == ReplyType.VIDEO_URL or reply.type == ReplyType.APP or reply.type == ReplyType.MINIAPP:
                    pass
        ...
        if reply and reply.type:
            ...
            if not e_context.is_pass() and reply and reply.type:
                ...
                if reply.type == ReplyType.TEXT and "//" in reply.content:
                    ...
                    segments_to_send = [s.strip() for s in all_segments if s.strip()]

                    if not segments_to_send:
                        logger.debug("[chat_channel] All segments are empty after splitting by //, skipping send.")
                        return

                    logger.debug(f"[chat_channel] Message will be split into {len(segments_to_send)} segments")
                    for i, segment_text in enumerate(segments_to_send):
                        logger.debug(f"[chat_channel] Sending segment {i+1}/{len(segments_to_send)}: {segment_text[:50]}...")
                        segment_reply = Reply(ReplyType.TEXT, segment_text)
                        self._send(segment_reply, context)
                        if i < len(segments_to_send) - 1:
                            # 添加随机延迟，模拟人类打字间隔
                            import random
                            delay = random.uniform(1.0, 3.0)  # 1到3秒的随机延迟，避免消息丢失
                            logger.debug(f"[chat_channel] Waiting {delay:.2f}s before sending next segment")
                            time.sleep(delay)
                else:
